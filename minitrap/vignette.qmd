...
- `.trap_safely(.f)`: This function wraps `.f` so that a call which would normally produce an error will instead return a list containing two elements named `result` and `error`. Exactly one of these two elements will be `NULL`, depending on whether the unmodified function would error when called the same way.
- `.trap_quietly(.f)`: This function wraps `.f` so that printed all output, warnings, and messages are silently captured instead of shown. The modified function returns a four element named list, with components for `result`, `output`, `warnings`, and `messages`.  
