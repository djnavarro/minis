...
|---|---|---|
| `.table_tibble(...)` | `tibble()` | Later columns can reference earlier ones by name |
| `.table_as_tibble(x, ...)` | `as_tibble()` | Just `as.data.frame()` |
| `.table_rownames_to_column(.data, var)` | `rownames_to_column()` | No-op if row names are absent or the default sequence |
| `.table_add_row(.data, ...)` | `add_row()` | Appends one row, matched by column name |
...
cross-column references (`.table_tibble(a = 1, b = a + 1)`).
...
sequential-evaluation logic lives directly inside `.table_tibble()` and
`.table_add_row()` rather than in a shared `...`-accepting helper, so
...
Also fixed relative to the original: `.table_rownames_to_column()` now
