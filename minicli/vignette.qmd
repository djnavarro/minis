...
- `.cli_col_black()`/`_red()`/`_green()`/`_yellow()`/`_blue()`/
  `_magenta()`/`_cyan()`/`_white()`/`_grey()` are used to generate output in a particular colour, which they achieve by wrapping a string in an ANSI colour code.
