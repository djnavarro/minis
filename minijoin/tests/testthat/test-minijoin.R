...
test_that("mjoin_inner_join() keeps only rows matching on both sides", {
  out <- mjoin_inner_join(bands, albums, by = "band")
  expect_equal(nrow(out), 1)
  expect_equal(out$band, "Beatles")
  expect_equal(out$album, "Abbey Road")
})

test_that("mjoin_left_join() keeps all rows of x, filling unmatched with NA, in x's order", {
  out <- mjoin_left_join(bands, albums, by = "band")
  expect_equal(nrow(out), 3)
  expect_equal(out$band, bands$band)
  expect_true(is.na(out$album[out$band == "Who"]))
})

test_that("mjoin_right_join() keeps all rows of y, filling unmatched with NA", {
  out <- mjoin_right_join(bands, albums, by = "band")
  expect_equal(nrow(out), 2)
  expect_true(any(is.na(out$founded)))
})

test_that("mjoin_full_join() keeps all rows from both sides", {
  out <- mjoin_full_join(bands, albums, by = "band")
...
test_that("joins support a named `by` for differently-named join columns", {
  ...
  out <- mjoin_inner_join(bands, albums2, by = c("band" = "artist"))
  expect_equal(out$album, "Abbey Road")
})

test_that("keep = TRUE preserves both join columns, suffixed", {
  out <- mjoin_left_join(bands, albums, by = "band", keep = TRUE)
...
test_that("na_matches = 'never' excludes NA-keyed rows from matching", {
  ...
  out_na <- mjoin_inner_join(x, y, by = "id", na_matches = "na")
  out_never <- mjoin_inner_join(x, y, by = "id", na_matches = "never")
